...
class CommentBatman extends Component {
    constructor(props) {
      super(props)
  ...
  submitHandler = e => {
      e.preventDefault()
      console.log(this.state, options)
      axios.post('https://api.noroff.dev/api/v1/social/posts/5256/comment', this.state, options)
      .then(response=>{
        console.log(response)
      })
      .then(response =>{
  ...
  render() {
    ...
    return (
      ...
      <Link className="back" to="/Batman">Back to profile</Link> 
      <FeedBannerTwo/>
