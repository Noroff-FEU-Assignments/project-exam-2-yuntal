...
import LikesBatman from "./LikePost";
...
function BatmanPost() {
 ...
 useEffect(function () {
  async function fetchData() {
   try {
    const response = await fetch(  'https://api.noroff.dev/api/v1/social/profiles/BruceWayne/posts', options
    );
 ...
 return (
  <>
   ...
   <Row>     
   <Col>
   <Card className="post-card" id="first-post-card"> 
   <Card.Img  variant="top"  className="my-avatar" src={post.media} alt="my-avatar"/>
   ...
   <Row xs={2} md={4} lg={6}>
   <LikesBatman/>
   <Link to="/CommentBatman">  <FaRegComment style={{color:'#fff', fontSize:'36px', marginTop:'2.5rem'}}/>  </Link>
   </Row>
   </Card.Body>
   </Card>
   </Col>
