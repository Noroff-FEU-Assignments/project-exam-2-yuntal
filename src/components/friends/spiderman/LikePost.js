...
class LikesPeter extends Component {
    constructor(props) {
      super(props)
    ...
    submitHandler = e => {
        ...
        axios.post('https://api.noroff.dev/api/v1/social/posts/5261/', this.state, options)
        .then(response=>{
  ...
  submitHandler = e => {
      e.preventDefault()
      console.log(this.state, options)
      axios.put('https://api.noroff.dev/api/v1/social/posts/5261/react/👍', this.state, options)
      .then(response=>{
        console.log(response)
      })
      .catch(error=>{
