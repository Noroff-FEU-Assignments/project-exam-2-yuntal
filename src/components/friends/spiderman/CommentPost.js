...
class CommentSpiderman extends Component {
    constructor(props) {
      super(props)
    ...
    submitHandler = e => {
        ...
        axios.get('https://api.noroff.dev/api/v1/social/posts/5261/', this.state, options)
        .then(response=>{
  ...
  submitHandler = e => {
      e.preventDefault()
      console.log(this.state, options)
      axios.post('https://api.noroff.dev/api/v1/social/posts/5261/comment', this.state, options)
      .then(response=>{
        console.log(response)
      })
      .then(response =>{
  ...
  render() {
    ...
    return (
      ...
      <Link className="back" to="/Spiderman" style={{color:'#fff'}}>Back to profile</Link>
      <FeedBannerTwo/>
      ...
      <BootstrapForm.Group className="mb-3">
        ...
        <Form.Control as="textarea" className="text-area" name="message" type="text" placeholder="Message"rows={3} style={{backgroundColor:'#212529', border:'2px solid #fff', marginTop:'-1rem', marginBottom:'2rem', height:'100px', color:'#fff'}} />
      </BootstrapForm.Group>
          <BootstrapForm.Group className="mb-3">
          <div>
...
export default CommentSpiderman;