...
class UnfollowPeter extends Component {
    constructor(props) {
      super(props)
    ...
    submitHandler = e => {
        ...
        axios.get('https://api.noroff.dev/api/v1/social/profiles/peter_parker', this.state, options)
        .then(response=>{
  ...
  submitHandler = e => {
      e.preventDefault()
      console.log(this.state, options)
      axios.put(`https://api.noroff.dev/api/v1/social/profiles/peter_parker/unfollow`, this.state, options)
      .then(response=>{
        console.log(response)
      })
      .then(response =>{
