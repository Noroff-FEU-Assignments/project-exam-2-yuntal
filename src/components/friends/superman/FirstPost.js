...
import LikeSuperman from "./LikePost"
...
function SupermanPost() {
 ...
 useEffect(function () {
  async function fetchData() {
   try {
    const response = await fetch(  'https://api.noroff.dev/api/v1/social/profiles/ClarkKent/posts', options);
...
<Card.Body id="post-body"> 
  ...
  <Row xs={2} md={4} lg={6}>
  <LikeSuperman/>
  <Link to="/CommentSuperman">  <FaRegComment className="fa-comment" style={{color:'#fff', fontSize:'36px', marginTop:'2.5rem'}}/>  </Link>
  </Row>
  </Card.Body>
</Card>
</Col>
