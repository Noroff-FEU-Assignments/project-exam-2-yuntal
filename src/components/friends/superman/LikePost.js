...
class LikeSuperman extends Component {
    constructor(props) {
      super(props)
    ...
    submitHandler = e => {
        ...
        axios.post('https://api.noroff.dev/api/v1/social/posts/5378/', this.state, options)
        .then(response=>{
  ...
  submitHandler = e => {
      e.preventDefault()
      console.log(this.state, options)
      axios.put('https://api.noroff.dev/api/v1/social/posts/5378/react/👍', this.state, options)
      .then(response=>{
        console.log(response)
      })
      .catch(error=>{
    ...
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }} >
        <form onSubmit={this.submitHandler} id="my-form">
            <div>
                <button className="like" type="submit"  name="likebutton" value={reactions}onClick={this.changeHandler}><FiThumbsUp className="fa-thumbs"/></button>
            </div>
        </form>
      </div>
    )
